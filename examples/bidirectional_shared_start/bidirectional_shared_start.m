...
% Wait for device to send start condition
while mcu_serial_port.BytesAvailable < n_rx_bytes
    % wait
end
fprintf('Starting! \n');
tic;                                    % start timing
...
ii = 1;
while toc < duration
    while mcu_serial_port.BytesAvailable < n_rx_bytes % wait for data (not necessary)
    end
